...
contract TimeUnit { 
  ...
  function setStartTime() public { 
    startTime = block.timestamp; //เวลาปัจจุบันตอนนี้
  } 
  ...
  function elapsedSeconds() public view returns (uint256) { 
    return (block.timestamp - startTime); //หน่วยเป็นวินาที
  } 
  ...
  function elapsedMinutes() public view returns (uint256) { 
    ...
    return (block.timestamp - startTime) / 1 minutes; //แปลงเป็นหน่วยนาที
  } 
}